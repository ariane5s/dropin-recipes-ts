...
class DROPinAPI {
    static enableLocalMode(port = 8040, address = "localhost", https = false) {
    ...
    static register(invitationCode, email, password, realm) {
    ...
    static login(email, password, realm) {
    ...
    static createLine(recipe, collection, data, version) {
        return this.request(FetchMethod.PUT, `recipes/${recipe}/collections/${collection}/lines`, {}, {
            data,
            version
        });
    }
    static updateLine(line, data) {
        return this.request(FetchMethod.POST, `lines/${line}`, {}, {
            data
        });
    }
    ...
    static getPermission(recipe, permission) {
        return this.request(FetchMethod.GET, `recipes/${recipe}/permissions/${permission}`);
    }
    static createPermission(recipe, permission, name, collections) {
        return this.request(FetchMethod.PUT, `recipes/${recipe}/permissions`, {}, {
            name,
            slug: permission,
            collections
        });
    }
    static updatePermission(recipe, permission, name, collections) {
        return this.request(FetchMethod.POST, `recipes/${recipe}/permissions/${permission}`, {}, {
            name,
            slug: permission,
            collections
        });
    }
    static deletePermission(recipe, permission) {
        return this.request(FetchMethod.DELETE, `recipes/${recipe}/permissions/${permission}`);
    }
}
