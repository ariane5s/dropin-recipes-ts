...
export const PriceFieldValidator = (data: PriceField | any, context?: l4rContext): Promise<Error[]> => {
  if(typeof data !== "object") {
    return Promise.resolve([ new Error(i18nValidator.dataMustBeAnObject, ErrorType.INCOMPATIBLE, context) ])
  }

  if(typeof data.number === "undefined") {
    return Promise.resolve([ new Error(i18nValidator.noDataFoundForKey("number"), ErrorType.INCOMPLETE, context) ])
  }

  if(typeof data.number !== "number") {
    return Promise.resolve([ new Error(i18nValidator.dataMustBeANumber, ErrorType.INCOMPATIBLE, context) ])
  }

  if(typeof data.currency === "undefined") {
    return Promise.resolve([ new Error(i18nValidator.noDataFoundForKey("currency"), ErrorType.INCOMPLETE, context) ])
  }

  const totalKeys = Object.keys(data).length
  if(totalKeys > 2) {
    return Promise.resolve([ new Error(i18nValidator.keysCountMismatch(totalKeys, 2), ErrorType.UNEXPECTED, context) ])
  }
